...
def eval(model, val_iter, is_save=False, phase='test', eopch=-1, mode=None):
    ...
    # emo_uar = f1_score(total_emo_label, total_emo_pred, average='micro')
    # int_uar = f1_score(total_int_label, total_int_pred, average='micro')
    ...
    return emo_acc, int_acc, emo_uar, int_uar, emo_cm, int_cm
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count,
        ...
        # emo_acc, int_acc, emo_uar, int_uar, int_cm = eval(model, val_dataset)
        logger.info('Val result of epoch %d / %d emo_acc %.4f int_acc %.4f emo_uar %.4f int_uar %.4f' % (
            epoch, opt.niter + opt.niter_decay, emo_acc, int_acc, emo_uar, int_uar))
        logger.info('\n{}'.format(emo_cm))
        logger.info('\n{}'.format(int_cm))
        ...
        if opt.has_test and opt.verbose:
            ...
            # emo_acc, int_acc, emo_uar, int_uar, int_cm = eval(model, tst_dataset)
            logger.info('Tst result of epoch %d / %d emo_acc %.4f int_acc %.4f emo_uar %.4f int_uar %.4f' % (
                epoch, opt.niter + opt.niter_decay, emo_acc, int_acc, emo_uar, int_uar))
            logger.info('\n{}'.format(emo_cm))
            logger.info('\n{}'.format(int_cm))

        # record epoch with best result
        if emo_uar * int_uar > best_emo_uar * best_int_uar:
            best_eval_epoch = epoch
            best_emo_uar = emo_uar
            best_emo_acc = emo_acc
            best_int_uar = int_uar
            best_int_acc = int_acc
        select_metric = 'uar'
        best_metric = emo_uar + int_uar
    ...
    if opt.has_test:
        ...
        # emo_acc, int_acc, emo_uar, int_uar, int_cm = eval(model, tst_dataset, is_save=True, phase='test')
        logger.info('Tst result emo_acc %.4f int_acc %.4f emo_uar %.4f int_uar %.4f' % (emo_acc, int_acc, emo_uar, int_uar))
        logger.info('\n{}'.format(emo_cm))
        logger.info('\n{}'.format(int_cm))
